...
create table if not exists public.colecciones (
  ...
  estado_publicacion text not null default 'activo'
    check (estado_publicacion in ('activo', 'desactivado')),
  creado_en timestamptz not null default now(),
  actualizado_en timestamptz not null default now()
);
...
create table if not exists public.productos (
  ...
  tipo_producto text not null
    check (tipo_producto in ('disponible', 'bajo_pedido', 'personalizado')),
  ...
  estado_publicacion text not null default 'borrador'
    check (estado_publicacion in ('borrador', 'activo', 'desactivado')),
  destacado boolean not null default false,
  mensaje_whatsapp text,
  creado_en timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
  constraint producto_stock_coherente
    check (controla_stock or stock_actual = 0)
);
...
create index if not exists productos_coleccion_id_idx
  on public.productos (coleccion_id);
...
create index if not exists productos_publicados_idx
  on public.productos (estado_publicacion, destacado, creado_en desc);

create index if not exists imagenes_producto_producto_id_orden_idx
  on public.imagenes_producto (producto_id, orden);
...
create trigger perfiles_actualizar_actualizado_en
...
create policy perfiles_ver_propio_o_administrativo
on public.perfiles for select
to authenticated
using (id = auth.uid() or private.usuario_es_administrador());

drop policy if exists colecciones_publicadas_publicas on public.colecciones;
create policy colecciones_publicadas_publicas
...
create policy colecciones_gestion_interna
on public.colecciones for all
to authenticated
using (private.usuario_es_administrador())
...
create policy categorias_gestion_interna
...
create policy productos_activos_publicos
on public.productos for select
to anon, authenticated
using (estado_publicacion = 'activo');

drop policy if exists productos_gestion_interna on public.productos;
create policy productos_gestion_interna
...
create policy imagenes_productos_activos_publicas
...
create policy imagenes_gestion_interna
on public.imagenes_producto for all
to authenticated
using (private.usuario_es_administrador())
with check (private.usuario_es_administrador());
;
